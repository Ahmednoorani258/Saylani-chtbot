...
app.post("/webhook" ,async (req, res) => {
  ...
  async function registration(agent) {
    ...
    // ✅ Immediately respond to Dialogflow to avoid timeout (within 5 sec)
    ...
    // ✅ Non-blocking email and WhatsApp tasks run in background
    setTimeout(async () => {
      try {
        const info = await transporter.sendMail({
          from: '"Saylani IT Registration" <ahmednoorani258@gmail.com>',
          to: email,
        ...
        const message = await client.messages.create({
          ...
          body: `👋 Hello ${studentname.name}, you’re registered for "${coursename}". We've emailed you confirmation at ${email}.`,
          to: `whatsapp:${process.env.TWILIO_PHONE_NUMBER}`,
        });
        console.log("✅ WhatsApp message sent:", message.sid);
      } catch (error) {
        console.error("❌ Email/WhatsApp error:", error);
      }
    }, 0);
  }
